...
class ApiListingCache(scanner: PlayApiScanner, config: PlaySwaggerConfig) extends Loggable {
  private val cache: mutable.Map[String, OpenAPI] = mutable.Map.empty

  def listing(host: String): OpenAPI = cache.getOrElseUpdate(
    host, {
      ...
      val reader = new PlayReader(swaggerConfig.openAPI(swagger), scanner.routes)
