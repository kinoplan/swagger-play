...
class SwaggerController @Inject() (components: ControllerComponents, swaggerPlugin: SwaggerPlugin)
  ...
  def getResourcesAsJson: Action[AnyContent] = Action { _ =>
    ...
    Results.Ok(ByteString(response)).as(ContentTypes.JSON).withHeaders(AccessControlAllowOrigin)
  }
  ...
  def getResourcesAsYaml: Action[AnyContent] = Action { _ =>
    ...
    Results.Ok(ByteString(response)).as("text/vnd.yaml").withHeaders(AccessControlAllowOrigin)
  }

  private def generateSwaggerYaml(openAPI: OpenAPI): String =
    try Yaml.pretty().writeValueAsString(openAPI)
    catch {
      case NonFatal(t) =>
  ...
  private def generateSwaggerJson(openAPI: OpenAPI): String =
    try Json.pretty().writeValueAsString(openAPI)
    catch {
      case NonFatal(t) =>
